...
class AffineNode(object):
    ...
    def backward(self):
        d_b = self.d_out
        d_x = np.matmul(np.transpose(self.W.out), self.d_out)
        d_W = np.matmul(self.d_out.reshape(-1, 1), np.transpose(self.x.out.reshape(-1,1)))
        self.b.d_out += d_b
        self.x.d_out += d_x
        self.W.d_out += d_W
        return self.d_out
...
class TanhNode(object):
    ...
    def backward(self):
        d_a = (1 - self.out * self.out) * self.d_out
        self.a.d_out += d_a
        return self.d_out
